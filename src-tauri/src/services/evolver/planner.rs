...
// =====================================================================
// OE2: Multi-candidate plan generation (MAP-Elites integration)
// =====================================================================

/// Generation budget per candidate in the population.
#[derive(Debug, Clone, Copy)]
pub struct CandidateConfig {
    /// Number of candidate plans to generate.
    pub n_candidates: usize,
    /// Random seed offset per candidate (to get diversity in LLM calls).
    pub seed_offset: u64,
}
...
/// A candidate plan with its fitness score.
#[derive(Debug, Clone)]
...
/// Generate N candidate plans in parallel and return them sorted by score (best first).
///
/// This implements the OE2 task from INTEGRATION_UNIFIED.md:
/// "Add `generate_population(N)` to planner.rs — multi-candidate plans"
///
/// Each candidate is generated by calling the LLM with a slightly different
/// system prompt (via seed offset in the user prompt), then all candidates
/// are scored and sorted. The best candidate is returned.
pub async fn generate_population(
    source_root: &Path,
    all_issues: Vec<Issue>,
    request: PlanRequest,
    api_key: Option<String>,
    ...
    let n = config.n_candidates.max(1).min(10); // cap at 10 to avoid excessive LLM calls

    // Generate N candidates in parallel using join_all
    let futures: Vec<_> = (0..n)
        .map(|i| {
            let source_root = source_root.to_path_buf();
            let issues = all_issues.clone();
            let request = request.clone();
            let api_key = api_key.clone();
            let diagnose_id = diagnose_id.clone();
            let seed_offset = config.seed_offset;

            async move {
                // Build with candidate index baked into the prompt seed
                let candidate_request = PlanRequest {
                    // Embed candidate index into risk_threshold to get diversity
...
/// Select the best candidate from a scored population.
pub fn select_best(scored: &[ScoredCandidate]) -> Option<&ScoredCandidate> {
    scored.first()
}

/// Score a plan for fitness comparison.
///
/// Higher scores = better plan. Scoring considers:
/// - Number of steps (prefer fewer steps, but not too few)
/// - Risk score (lower risk preferred)
/// - Expected impact quality
fn score_plan(plan: &Plan) -> f64 {
    let step_count = plan.steps.len() as f64;
    let risk = plan.risk_score as f64;

    // Base score: reward having steps (plans that do something)
    // but penalize excessive complexity
    let complexity_score = if step_count == 0.0 {
        -5.0 // trivial plan with no steps
    } else {
        (4.0 - (step_count - 1.0).abs()).clamp(0.0, 4.0)
    };

    // Risk penalty: lower risk is better
    // risk is 0..1, so (1.0 - risk) gives 1 for no risk, 0 for max risk
    let risk_score = 1.0 - risk;

    // Impact quality: longer impact strings suggest more thought
    let impact_len = plan.expected_impact.len() as f64;
    let impact_score = (impact_len / 50.0).clamp(0.0, 2.0); // normalize to ~50 chars = good

    // Mode bonus: LLM-generated plans preferred over trivial
    let mode_bonus = if plan.mode == "llm" { 1.0 } else { 0.0 };

    complexity_score + (risk_score * 2.0) + impact_score + mode_bonus
}

/// Plan with metadata for MAP-Elites grid insertion.
#[derive(Debug, Clone)]
pub struct PlanWithDescriptors {
    pub plan: Plan,
    /// Complexity descriptor [0, 1] — normalized step count
    pub complexity: f64,
    /// Diversity descriptor [0, 1] — based on step type distribution
    pub diversity: f64,
}

impl PlanWithDescriptors {
    /// Compute behavioral descriptors for MAP-Elites grid placement.
    pub fn from_plan(plan: &Plan) -> Self {
        let step_count = plan.steps.len().max(1) as f64;

        // Complexity: normalized step count (0-5 steps = low, 6-10 = medium, 11+ = high)
        let complexity = ((step_count - 1.0) / 10.0).clamp(0.0, 1.0);

        // Diversity: based on the variety of step types
        let mut type_counts = std::collections::HashMap::new();
        for step in &plan.steps {
            let t = match step {
                PlanStep::EditFile { .. } => "edit",
                PlanStep::CreateFile { .. } => "create",
                PlanStep::RunCommand { .. } => "command",
            };
            *type_counts.entry(t).or_insert(0) += 1;
        }
        let unique_types = type_counts.len() as f64;
        let diversity = (unique_types / 3.0).clamp(0.0, 1.0); // 3 step types max
    ...
    /// Return descriptors array for MAP-Elites grid [complexity, diversity].
    pub fn descriptors(&self) -> [f64; 2] {
        [self.complexity, self.diversity]
    }
}
