...
pub struct MemoryService {
    ...
    /// Hotness tracker for retrieval ranking. Tracks per-event access
    /// frequency so recently/frequently accessed events rank higher.
    /// Initialized lazily; use `access_tracker_mut()` to access.
...
impl MemoryService {
    ...
    pub fn init(_app: &tauri::AppHandle) -> Result<Arc<Self>, MemoryError> {
        ...
        Ok(Arc::new(Self {
            ...
            l3,
            graph,
            access_tracker: RwLock::new(AccessTracker::new(1000)),
    ...
    /// Get the current L0 abstract — a human-readable rolling summary
    /// of recent important L1 events. Generated by `consolidation::run()`
    /// and cached in `lifecycle_state.json`. Used to prepend context
    /// to the system prompt so the agent "remembers" recent facts.
    pub fn get_l0_abstract(&self) -> String {
        consolidation::get_l0_abstract(self)
    }

    /// Snapshot L0 for inclusion in a future system-prompt prefix.
    /// Returns the rolling L0 abstract + the raw L0 ChatMsg list.
    pub fn snapshot_working(&self) -> (String, Vec<ChatMsg>) {
        let abstract_text = self.get_l0_abstract();
        let messages = self.l0.read().snapshot();
        (abstract_text, messages)
    }

    // ---- Access tracking (M4) ----

    /// Bump access count for an event id. Called after each recall
    /// hit so hotness scores reflect real retrieval patterns.
    pub fn bump_access(&self, event_id: &str) {
        let mut tracker = self.access_tracker.write();
        let now = now_ms();
        tracker.bump(event_id, now);
        // Prune stale entries every 100 bumps to bound memory.
        tracker.prune_stale(3_600_000, now); // 1h stale threshold
    }

    /// Get hotness score for an event id. Used by retrieval ranking.
    pub fn hotness_score(&self, event_id: &str, event_ts: i64) -> f32 {
        let tracker = self.access_tracker.read();
        tracker.hotness(event_id, event_ts, now_ms())
    }

    /// Clone a snapshot of the access tracker. Used by retrieval ranking
    /// to score events by recency/frequency. The snapshot is cheap (just
    /// a clone of the HashMap + counter, no I/O).
    pub fn access_tracker(&self) -> AccessTracker {
        self.access_tracker.read().clone()
    }
