...
impl<R: Runtime> Stt<R> {
    fn models_dir(&self) -> PathBuf {
        // LOCAL PATCH (Luna Agent): prefer a folder owned by the running
        // app rather than polluting `%APPDATA%` (which the user reasonably
        // considers "system space"). Lookup order:
        //   1. `LUNA_WHISPER_MODELS_DIR` env var — explicit override, useful
        //      for portable installs and CI.
        //   2. The folder holding the executable (`resource_dir`) — what the
        //      user sees as "the app's folder". In dev this is
        //      `src-tauri/target/<profile>/`; in prod it's the install dir.
        //   3. `app_local_data_dir()` (per-user, non-roaming) — the
        //      conventional place for app data on Windows when
        //      `resource_dir` isn't writable (e.g. `Program Files`).
        //   4. Current working directory — last-ditch fallback so we never
        //      silently drop a model on the floor.
        ...
        if let Ok(resource_dir) = self.app.path().resource_dir() {
            return resource_dir.join("whisper-models");
        }
        if let Ok(local_data) = self.app.path().app_local_data_dir() {
            return local_data.join("whisper-models");
        }
        std::env::current_dir()
